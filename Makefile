...
ifeq ("$(CLIENT)","")
	...
	(cd $(DISTRIB_DIR); $(TAR) zcf ../$(DISTRIB).tar.gz \
			$(EXCLUDE) \
			$(DISTRIB)/configure \
			$(DISTRIB)/ocamlgraph.tar.gz \
	                $(DISTRIB)/bin $(DISTRIB)/lib/plugins \
	                $(DISTRIB)/lib/gui $(DISTRIB)/external \
			$(addprefix $(DISTRIB)/,$(DISTRIB_FILES))\
			$(addprefix $(DISTRIB)/,$(DISTRIB_TESTS))\
			$(DISTRIB)/tests/non-free \
	)
