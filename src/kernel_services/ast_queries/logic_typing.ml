...
struct
  ...
  let apply_substs types functions _lemmas loc inc_loc ga =
    (* For typechecking, we need original functions but with new types *)
    let funcs_with_new_types = ref [] in
    let modified_global = Cil.visitCilGlobal (object(_self)
      ...
      method! vglob t =
        ...
        | GAnnot (Dfun_or_pred (_, _), _) ->
            DoChildrenPost (fun x -> List.iter (function
              | GAnnot (Dfun_or_pred (info, _), _loc) ->
        ...
        | GAnnot (Dlemma (_, _, _, _, _, _, _), _) ->
            DoChildrenPost (fun x -> x |> List.map (function
              | GAnnot (Dlemma (name, is_axiom, labels, ss, pred, attr, _loc), _aloc) ->
    ...
    end) (GAnnot (ga, loc)) |> List.map (function (GAnnot (x, _)) -> x |  _ -> assert false) in
  ...
  let apply_forward_type_substs types functions loc ga =
    Cil.visitCilGlobal (object(_self)
      inherit Cil.genericCilVisitor (Visitor_behavior.inplace ())
      ...
      method! vglob t =
        ...
        | GAnnot (Dfun_or_pred (dst_fun, _loc), _aloc) ->
  ...
  end) (GAnnot (ga, loc)) |> List.map (function (GAnnot (x, _)) -> x | _ -> assert false)
  ...
  let rec c_mk_cast ?(force=false) ?overflow e oldt newt =
    ...
    if is_same_c_type oldt newt then begin
      if force then
        Logic_utils.mk_cast ~loc ~force ?overflow newt e
      else e
    end else begin
      ...
      if isPointerType newt && isLogicNull e && not (isLogicZero e) then
        ...
        (if force then
           Logic_const.term ~loc (TCastE (newt, Check, e)) (Ctype newt)
         else
        ...
        match Cil.unrollType newt, e.term_node with
          ...
          | _ ->
              { e with term_node = (Logic_utils.mk_cast ~force ?overflow newt e).term_node;
  ...
  let rec mk_cast ?(explicit=false) e newt =
    ...
    if is_same_type e.term_type newt then begin
      if explicit then begin
        ...
        | Ctype cnewt ->
          { e with term_node = TCastE(cnewt,Check,e); term_type = newt }
        | _ -> e
      end else e
      ...
      | t1, Ctype t2 when Logic_const.is_boolean_type t1
        ...
        Logic_const.term ~loc (TCastE (t2,Check,e)) newt
  ...
  *)
  ...
  let update_predicate_wrt_label p lab =
    match p.pred_content with
      ...
      | _ -> { p with pred_content = Pat(p,lab) }
  ...
  let check_lval_kind m t =
    let rec aux t = match t.term_node with
      ...
      | TAlignOfE _ | TUnOp (_,_) | TBinOp (_,_,_) | TCastE (_,_,_)
      | TStartOf _ | Tlambda (_,_) | TDataCons (_,_) | Tbase_addr (_,_)
      | Toffset (_,_) | Tblock_length (_,_) | Tnull | Tapp _ | TCoerce (_,_)
      | TCoerceE (_,_) | TUpdate (_,_,_) | Ttypeof _ | Ttype _ | TOffsetOf _ | Toffset_min _ | Toffset_max _->
        false
    in
    aux t
  ...
  and term_node ctxt env loc pl =
    ...
    | PLoffsetof (lt, member) ->
        (match Logic_utils.unroll_type ~unroll_typedef:false (logic_type ctxt loc env lt) with
           | Ctype ct when isStructOrUnionType ct ->
             (match unrollType ct with
              | TComp (ci, _, _) ->
                ...
                in
                begin match ctxt.find_comp_field ci member with
                  | off ->
                    let iter_fields f =
                      let rec loop =
                        function
                    ...
                    loop off
                  | exception Not_found ->
                    ctxt.error loc "there is no field `%s' in %s" member (compFullName ci)
                end
              | t ->
                ctxt.error loc "%a is not a structure or union type (but %a)"
                  Cil_printer.pp_typ ct Cil_printer.pp_typ t)
           | _ -> ctxt.error loc "offsetof can only handle C types")
    ...
    | PLbinop (t1, (Badd | Badd_mod | Bsub | Bsub_mod | Bmul | Bmul_mod | Bdiv | Bdiv_mod | Bmod | Bmod_mod
        ...
        let binop op tr = TBinOp (op, mk_cast t1 tr, mk_cast t2 tr), logic_arithmetic_promotion tr in
        let binop_mod op =
          let try_integral_cast lt t =
            match unroll_type lt with
            | Ctype ty -> ctxt.integral_cast ty t
    ...
    | PLoffset_max (l, t) | PLoffset_min (l, t) ->
        ...
        if isLogicPointer t then
          ...
          in
          t.term_node, t.term_type
        else ctxt.error loc "subscripted value is neither array nor pointer"
    | PLblock_length (l, t) ->
    ...
    | PLcast (ty, t) | PLcast_mod (ty, t) ->
        ...
        (match unroll_type ~unroll_typedef:false (logic_type ctxt loc env ty) with
          | (Ctype tnew) as ctnew ->
            let overflow =
              match pl with
                | PLcast_mod _ when is_integral_type t.term_type && is_integral_type ctnew -> Modulo
                | PLcast_mod _ ->
                    ctxt.error loc "modulo casts are applicable to integral types only, got types `%a' and `%a'"
            ...
            (match t.term_type with
              | Ctype told ->
                ...
                (c_mk_cast ~force:true ~overflow t told tnew).term_node , ctnew
    ...
    | PLxor (t1,t2) ->
        let ty = Ltype (ctxt.find_logic_type Utf8_logic.boolean,[]) in
  ...
  let predicate ctxt env p0 =
    ...
    | PLlet(x,def,body) ->
        ...
        { pred_name = []; pred_loc = p0.lexpr_loc;
          pred_content = Plet(var,tbody) }
    | PLcast _ | PLcast_mod _ | PLblock_length _ | PLbase_addr _ | PLoffset _ | PLoffset_max _ | PLoffset_min _
    | PLarrget _ | PLlist _ | PLrepeat _ | PLarrow _
 ...
 let type_from ctxt ~accept_formal env (l,d) =
  ...
  let model_annot =
    ...
    let rec pre_translate_lt =
      let dummy_struct = Cil_const.mkCompInfo true "dummy" (fun _ -> []) [] in
      let dummy_union = Cil_const.mkCompInfo false "dummy" (fun _ -> []) [] in
      fun loc ->
        function
        | LTvoid  -> TVoid []
        | LTint ik -> TInt (ik, [])
        ...
        | LTnamed (s, []) -> TNamed ({ torig_name = s;
                                       ...
                                       ttype = TComp ({ dummy_struct with corig_name = s; cname = s },
                                                      ...
                                                      []);
                                       treferenced = false }, [])
        | LTattribute (lt, a) -> typeAddAttributes [a] @@ pre_translate_lt loc lt
        | LTnamed _
        | LTinteger
        | LTreal
        | LTarrow _ -> C.error loc "model fields can only be attached to C types"
    in
    fun ~stage loc ti ->
      let finish_with infos =
      ...
      | `Types ->
        ...
        let model_for_type = plain_logic_type loc (Lenv.empty ()) ti.model_for_type |> c_type_of loc in
        infos.mi_base_type <- model_for_type;
        infos.mi_field_type <- plain_logic_type loc (Lenv.empty ()) ti.model_type;
        finish_with infos
      | `Bodies ->
        let model_for_type = plain_logic_type loc (Lenv.empty ()) ti.model_for_type |> c_type_of loc in
        let infos = Logic_env.find_model_field ti.model_name model_for_type in
  ...
  *)
  ...
  let typedef ~stage loc env my_info = function
    ...
    | TDsyn typ when stage = `Types -> LTsyn (plain_logic_type loc env typ)
    | TDsyn _ (* `Bodies *) -> Extlib.the my_info.lt_def

  let rec annot ~stage in_axiomatic a =
            ...
            let l = List.map (fun x -> match x.decl_node with
                | LDinclude (name,types,functions,lemmas) ->
                    ...
                    if Logic_env.Axiomatics.mem name then (
                      ...
                      let[@warning "-8"] Daxiomatic (_, decls, _, _) = Logic_env.Axiomatics.find name in
                      List.map (fun dec ->
                        (* TODO add i to loc *)
  ...
  let annot ~stage a =
    start_transaction ();
    let res = annot ~stage 	false a in
    finish_transaction (); res

  let annot ~stage = C.on_error (annot ~stage) rollback_transaction
