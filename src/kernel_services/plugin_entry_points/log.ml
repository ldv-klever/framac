...
let add_source buffer = function
  ...
  | Some src ->
      let s =
        let col =
          let col = Filepath.(src.pos_cnum - src.pos_bol) in if col >= 0 then col else 0
        in
        Printf.sprintf "%s:%d:%d: "
          (Filepath.Normalized.to_pretty_string src.Filepath.pos_path) src.Filepath.pos_lnum col
      in
