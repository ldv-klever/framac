...
class cil_printer () = object (self)
  ...
  method location fmt loc =
    ...
    Format.fprintf fmt "%s:%d,%d-%d,%d"
      (Normalized.to_pretty_string loc1.pos_path)
      loc1.pos_lnum
      loc1.pos_cnum
      loc2.pos_lnum
      loc2.pos_cnum
  ...
  *)
  ...
  method attribute fmt = function
    | Attr(an, args) ->
      ...
      (match an, args with
       ...
       | ("_Atomic" | "Atomic"), [] -> fprintf fmt "_Atomic"; false
       | "missingproto", [] -> 
  ...
  method file fmt file =
    pp_set_margin fmt state.line_length;
    let fs = pp_get_formatter_out_functions fmt () in
    if state.line_directive_style <> None then begin
      let nl = ref false and ind = ref false in
      let weaken_nl f = nl := f && !nl in
      let fs' =
        { fs with
          out_string =
            (fun s p n ->
                 ...
                 (n = 0 ||
                  try
                    String.iteri
                      (fun i  ->
                         function ' ' | '\x0C' | '\n' | '\r' | '\t' -> ()
                                | _ when p <= i && i < p + n -> raise Exit
                                | _ -> ())
                      s; true
                  with Exit -> false);
               fs.out_string s p n);
          out_newline = (fun () -> if not !nl || not !ind then (fs.out_newline (); ind := false); nl := true);
          out_indent = (fun n -> if not !ind then fs.out_indent n; nl := true; ind := true)
        }
      in
      pp_set_formatter_out_functions fmt fs'
    end;
    fprintf fmt "@[/* Generated by Frama-C */@\n" ;
    print_std_includes fmt file.globals;
    Cil.iterGlobals file (fun g -> self#global fmt g);
    fprintf fmt "@]@.";
    if state.line_directive_style <> None then pp_set_formatter_out_functions fmt fs
