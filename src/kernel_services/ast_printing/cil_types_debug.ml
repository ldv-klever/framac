...
and pp_ikind fmt = function
  ...
  | I128 -> Format.fprintf fmt "I128"
  | IU128 -> Format.fprintf fmt "IU128"
...
and pp_ofl fmt : overflow_treatment -> _ = function
  | Check -> ()
  | Modulo -> Format.fprintf fmt "%%"
...
and pp_exp_node fmt = function
  ...
  | CastE(typ,ofl,exp) -> Format.fprintf fmt "CastE(%a,%a,%a)"       pp_typ typ  pp_ofl ofl pp_exp exp
...
and pp_unop fmt = function
  | Neg ofl -> Format.fprintf fmt "Neg%a" pp_ofl ofl
  | BNot -> Format.fprintf fmt "BNot"
  | LNot -> Format.fprintf fmt "LNot"

and pp_binop fmt = function
  | PlusA ofl -> Format.fprintf fmt "PlusA%a" pp_ofl ofl
  | PlusPI -> Format.fprintf fmt "PlusPI"
  | IndexPI -> Format.fprintf fmt "IndexPI"
  | MinusA ofl -> Format.fprintf fmt "MinusA%a" pp_ofl ofl
  | MinusPI -> Format.fprintf fmt "MinusPI"
  | MinusPP -> Format.fprintf fmt "MinusPP"
  | Mult ofl -> Format.fprintf fmt "Mult%a" pp_ofl ofl
  | Div ofl -> Format.fprintf fmt "Div%a" pp_ofl ofl
  | Mod ofl -> Format.fprintf fmt "Mod%a" pp_ofl ofl
  | Shiftlt ofl -> Format.fprintf fmt "Shiftlt%a" pp_ofl ofl
...
and pp_stmtkind fmt = function
  ...
  | AsmGoto(attributes,string_list,outs,ins,clobs,labs,location) ->
    Format.fprintf fmt "AsmGoto(%a,%a,%a,%a)"  pp_attributes attributes  (pp_list pp_string) string_list
      (pp_option pp_extended_asm) (Some (outs, ins, clobs, labs))   pp_location location
...
and pp_instr fmt = function
  ...
  | Asm(attributes,string_list,outs,ins,clobs,labs,location) ->
...
and pp_extended_asm fmt (asm_outputs, asm_inputs, asm_clobbers, asm_gotos) =
  Format.fprintf fmt "{asm_outputs=%a;asm_inputs=%a;asm_clobbers=%a;asm_gotos=%a}"
    (pp_list (pp_tuple3 (pp_option pp_string) pp_string pp_lval)) asm_outputs
    (pp_list (pp_tuple3 (pp_option pp_string) pp_string pp_exp)) asm_inputs
    (pp_list pp_string) asm_clobbers
    (pp_list (pp_ref pp_stmt)) asm_gotos
...
and pp_term_node fmt = function
  ...
  | TCastE(typ,ofl,term) -> Format.fprintf fmt "TCastE(%a,%a,%a)"  pp_typ typ  pp_ofl ofl pp_term term
  ...
  | Toffset_max (logic_label, term) ->
    Format.fprintf fmt "Toffset_max(%a,%a)"  pp_logic_label logic_label
      pp_term term
  | Toffset_min (logic_label, term) ->
    Format.fprintf fmt "Toffset_min(%a,%a)"  pp_logic_label logic_label
      pp_term term
  | TOffsetOf fieldinfo -> Format.fprintf fmt "TOffsetOf(%a)" pp_fieldinfo fieldinfo
  | Tpif (predicate,term1,term2) ->
    Format.fprintf fmt "Tpif(%a,%a,%a)"
      pp_predicate predicate pp_term term1 pp_term term2
