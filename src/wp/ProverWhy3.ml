...
object(self)
  ...
  method link e f =
    match Lang.link e f with
      | Engine.F_call   s ->
        Engine.F_call  (self#basename s)
      | Engine.F_left  (s1,               s2) ->
        Engine.F_left  (self#basename s1, self#basename s2)
      | Engine.F_right (s1,               s2) ->
        Engine.F_right (self#basename s1, self#basename s2)
      | Engine.F_assoc  s ->
        Engine.F_assoc (self#basename s)
...
object(self)
  ...
  method on_theory = function
    | "qed" | "driver" -> ()
    | "cint"   -> self#add_import ("cint","Cint")
    | "cbits"  -> self#add_import ("cint","Cint")
    | "cfloat" -> self#add_import ("cfloat","Cfloat")
    | "vset"   -> self#add_import ("vset","Vset")
    | "memory" -> self#add_import ("memory","Memory")
    | "cmath"  -> self#add_import ("cmath","Cmath")
    | thy -> Wp_parameters.fatal
	~current:false "No builtin theory '%s' for why3" thy

  method add_library path thy =
    self#add_import (thy,String.capitalize thy) ;
    self#add_dfile path
  ...
  method add_extlib file =
    ...
    self#add_library path thy

  method on_library thy =
    let path = Wp_parameters.find_lib (thy ^ ".why") in
    self#add_library path thy
  ...
  let file = match index with
    ...
    | Wpo.Function (kf,_behv) ->
      ...
      begin if age < Wpo.age wpo then
        ...
        let on_goal fmt wpo =
        (** iter on all the goal of a kf unfortunately not just the one of the
            current model *)
          ...
          if Model.S.equal wpo.Wpo.po_model model then begin
            ...
            | Wpo.GoalLemma _vca -> assert false
          end in
...
object(why)
  ...
  method result r =
    if r = 0 && not valid && Wp_parameters.UnsatModel.get () then
      begin
	let message =
          Pretty_utils.sfprintf "Model for %a" WpPropId.pretty pid in
