...
struct
  ...
  let sigma_any w = match w.sigma with
    | None -> Sigma.create ()
    | Some s -> Sigma.havoc_any s
  ...
  let use_assigns wenv stmt hpid ainfo wp = in_wenv wenv wp 
	...
	| Writes froms ->
	    ...
	    match outcome with
		  ...
		  let vcs = 
		    do_assigns ~source:FromCode ~descr:"Call Assigns"
  ...
  let cc_havoc d s = match d with
    | None -> { pre = Sigma.havoc_any s ; post = s }
  ...
  let make_vcqs target tags vc =
    ...
    let hyps = Conditions.freeze vc.hyps in
