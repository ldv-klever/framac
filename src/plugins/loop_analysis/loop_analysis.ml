  ...
  and transfer_exp exp load = match exp.enode with
    ...
    | Lval lval ->
      (match load lval with
       | None -> Unknown
       | Some(v) -> v)
    | BinOp(PlusA _,e1,e2,_) -> add (transfer_exp e1 load) (transfer_exp e2 load)
    | BinOp(MinusA _,e1,e2,_) ->
      add (transfer_exp e1 load) (neg (transfer_exp e2 load))
    | CastE(_,_,e) -> transfer_exp e load
...
module Store(* (B:sig *)
  ...
  let compile_node stmt (mem,conds,stmt2) =
    ...
    | Loop _ | Goto _ | AsmGoto _ | Break _ | Continue _ | Block _ | UnspecifiedSequence _ ->
