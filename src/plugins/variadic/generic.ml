...
let translate_va_builtin caller inst =
  ...
  let translate_va_arg () =
    ...
    | [{enode=Lval va_list};
       {enode=SizeOf typ};
       {enode=CastE(_, _, {enode=AddrOf lval})}] -> va_list, typ, lval
    ...
    let mk_cast exp typ = Cil.mkCast ~force:false ~overflow:Check ~e:exp ~newt:typ in 
...
let translate_call ~fundec stmt =
  ...
  let new_arg = Cil.mkCast ~force:false ~overflow:Check ~e:exp_vargs ~newt:(vpar_typ []) in
