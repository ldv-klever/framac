...
module Rewriting = struct
  ...
  let rec rewrite evaluate expr =
    ...
    | BinOp ((PlusA _ | MinusA _ as binop), e1, e2, typ) ->
      let op = if binop = PlusA Check || binop = PlusA Modulo then Add else Sub in
      let rewrite_binop typ v1 v2 =
        let inverse_op = if binop = PlusA Check || binop = PlusA Modulo then Arith.sub else Arith.add in
        let add_v2 var =
          { var with coeff = inverse_op typ var.coeff v2 }
        in
        let add_v1 var =
          let var = if binop = MinusA Check || binop = MinusA Modulo then neg var else var in
