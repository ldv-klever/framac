...
let warn_float_to_int_overflow warn_mode mn mx _msg =
  do_warn warn_mode.others
    (fun () ->
      match get_syntactic_context () with
	| ki, SyUnOp e ->
	  let aux lower bound =
            Extlib.may_map ~dft:() (fun n ->
              register_alarm emitter ki (Alarms.Float_to_int(e, n, lower))
                (fun annot k ->
                  k "@[overflow@ in conversion@ from@ floating-point@ \
                    to integer.@ %a@]%t" pr_annot annot)) bound
	  in
...
let warn_index warn_mode ~positive ~range:_ =
  do_warn warn_mode.others
    (fun () ->
      match get_syntactic_context () with
	| _,SyNone -> ()
	...
	| ki ,SyBinOp (_, IndexPI, e1, e2) ->
           let warn a =
             register_alarm emitter ki a
               (fun annot k ->
                 k "@[accessing out of bounds index.@ @[%a@]@]%t" pr_annot annot)
           in
...
let warn_nan_infinite warn_mode fkind _pp =
  ...
  do_warn warn_mode.others
	...
	| ki,SyUnOp (exp_r) ->
	   ...
	   register_alarm emitter ki
             ...
             (fun annot k -> k "@[non-finite@ %s@ value.@ %a@]%t"
               sfkind pr_annot annot))
...
let warn_overlap _msg warn_mode =
  do_warn warn_mode.others
    (fun () ->
      match get_syntactic_context () with
	| _,SyNone -> ()
	...
	| ki,SySep(lv1,lv2) ->
	   register_alarm emitter ki (Alarms.Overlap(lv1, lv2))
             (fun annot k ->
               k "@[partially overlapping@ lvalue assignment.@ %a@]%t"
                 pr_annot annot))
