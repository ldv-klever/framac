...
let backward_add_float fk ~res_value ~v1 ~v2 pos =
  try
    ...
    in
    let v1' = add (if pos then MinusA Check else PlusA Check) res_1ulp v2
    and v2' =
      if pos
      then add (MinusA Check) res_1ulp v1
      else add (MinusA Check) v1 res_1ulp
    in
    Some (v1', v2')
...
let backward_binop ~typ_res ~res_value ~typ_e1 v1 binop v2 =
  ...
  | PlusA _, TInt _ ->  backward_add_int typ ~res_value ~v1 ~v2 true
  | MinusA _, TInt _ -> backward_add_int typ ~res_value ~v1 ~v2 false

  | PlusA _, TFloat (fk, _) ->  backward_add_float fk ~res_value ~v1 ~v2 true
  | MinusA _, TFloat (fk, _) -> backward_add_float fk ~res_value ~v1 ~v2 false
  ...
  | (Shiftrt | Shiftlt _), TFloat _ -> None
