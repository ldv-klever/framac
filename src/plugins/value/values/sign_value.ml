...
let forward_binop _ op v1 v2 =
  match op with
  | PlusA  _ -> `Value (plus v1 v2)
  | MinusA _ -> `Value (plus v1 (neg_unop v2))
  | Mult   _ -> `Value (mul v1 v2)
  | Div    _ -> if equal zero v2 then `Bottom else `Value (div v1 v2)
  | _        -> `Value top
