...
let forward_unop ~context:_ _ op v =
  match op with
  | Neg _ -> `Value (neg_unop v), Alarmset.all
...
let forward_binop ~context _ op v1 v2 =
  match op with
  | PlusA  _ -> `Value (plus v1 v2), Alarmset.all
  | MinusA _ -> `Value (plus v1 (neg_unop v2)), Alarmset.all
  | Mult _   -> `Value (mul v1 v2), Alarmset.all
  | Div _    -> div_with_alarms context.right_operand v1 v2
  | _        -> `Value top, Alarmset.all
