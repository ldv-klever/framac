...
let classify loop_effect lval =
  let rec is_const_expr expr =
    ...
    | UnOp (_, e, _) | CastE (_, _, e) | Info (e, _) -> is_const_expr e
...
let rec get_lvalues expr =
  match expr.enode with
  | Lval lval -> [ lval ]
  | UnOp (_, e, _) | CastE (_, _, e) | Info (e, _) -> get_lvalues e
...
module Make (Abstract: Abstractions.Eva) = struct
  ...
  let rec delta_assign lval delta expr =
    ...
    let rec is_lval e = match e.enode with
      ...
      | CastE (typ, _, e) -> Cil.isIntegralType typ && is_lval e
      | Info (e, _) -> is_lval e
      | _ -> false
    in
    match expr.enode with
    | BinOp ((PlusA _ | MinusA _) as binop, e1, e2, _) ->
      if is_lval e1
      then add_to_delta binop delta e2
      else if is_lval e2 && (binop = PlusA Check || binop = PlusA Modulo)
      then add_to_delta binop delta e1
      else raise NoIncrement
    | CastE (typ, _, e) when Cil.isIntegralType typ -> delta_assign lval delta e
  ...
  let is_bounded_loop state limit loop_block =
    ...
    let value = binop (PlusA Check) v_init (binop (Mult Check) limit v_delta) in
