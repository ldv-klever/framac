...
module rec Transfer
  ...
  let rec base_addr_node = function
    ...
    | BinOp((MinusPP | PlusA _ | MinusA _ | Mult _ | Div _ | Mod _ | Shiftlt _ | Shiftrt
  ...
  and register_term kf varinfos term = match term.term_node with
    ...
    | Toffset_max _
    | Toffset_min _ -> Error.not_yet "Jessie-specific offsets"
  ...
  let register_object kf state_ref = object
    ...
    | Psubtype _ -> Error.not_yet "AstraVer subtype"
    ...
    | TOffsetOf _ | Tpif _ | Toffset_min _|Toffset_max _ ->
...
and apply_on_vi_base_from_exp f bhv ?kf ?stmt e = match e.enode with
  ...
  | Info(e, _) | CastE(_, _, e) -> apply_on_vi_base_from_exp f bhv ?kf ?stmt e
  | BinOp((PlusA _ | MinusA _ | Mult _ | Div _ | Mod _ |Shiftlt _ | Shiftrt | Lt | Gt | Le
