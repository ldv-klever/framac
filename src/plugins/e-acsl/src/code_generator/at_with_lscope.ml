...
let rec sizes_and_shifts_from_quantifs ~loc kf lscope sizes_and_shifts =
  ...
  | Lscope.Lvs_quantif(tmin, rel1, lv, rel2, tmax) :: lscope' ->
    let t_size = Logic_const.term ~loc (TBinOp(MinusA Check, tmax, tmin)) Linteger in
    let t_size = match rel1, rel2 with
      | Rle, Rle ->
        Logic_const.term
          ~loc
          (TBinOp(PlusA Check, t_size, Cil.lone ~loc ()))
          Linteger
      | Rlt, Rle | Rle, Rlt ->
        t_size
      | Rlt, Rlt ->
        Logic_const.term
          ~loc
          (TBinOp(MinusA Check, t_size, Cil.lone ~loc ()))
          Linteger
      | _ ->
    ...
    let t_shifted = match rel1 with
      | Rle ->
        Logic_const.term ~loc (TBinOp(MinusA Check, t_lv, tmin)) Linteger
      | Rlt ->
        let t =  Logic_const.term ~loc (TBinOp(MinusA Check, t_lv, tmin)) Linteger in
        Logic_const.term ~loc (TBinOp(MinusA Check, t, Cil.lone ~loc())) Linteger
      | _ ->
        Options.fatal "Unexpected comparison operator"
    in
    (* Returning *)
...
let size_from_sizes_and_shifts ~loc = function
  ...
  | (size, _) :: sizes_and_shifts ->
    List.fold_left
      (fun t_size (t_s, _) ->
        Logic_const.term ~loc (TBinOp(Mult Check, t_size, t_s)) Linteger)
      size
      sizes_and_shifts
...
let index_from_sizes_and_shifts ~loc sizes_and_shifts =
  let product terms = List.fold_left
    (fun product t ->
      Logic_const.term ~loc (TBinOp(Mult Check, product, t)) Linteger)
    (Cil.lone ~loc ())
    terms
  in
  let sum, _ = List.fold_left
    (fun (index, sizes) (t_size, t_shifted) ->
      ...
      let bi_mult_pi_beta_j =
        Logic_const.term ~loc (TBinOp(Mult Check, t_shifted, pi_beta_j)) Linteger
      in
      let sum = Logic_const.term
        ~loc
        (TBinOp(PlusA Check, bi_mult_pi_beta_j, index))
        Linteger
      in
      sum, t_size :: sizes)
    (Cil.lzero ~loc (), [])
...
let to_exp ~loc kf env pot label =
  ...
  let vi_at, e_at, env = Env.new_var
    ...
    (fun vi e ->
      ...
      let t_size =
        Logic_const.term ~loc (TBinOp(Mult Check, t_sizeof, t_size)) lty_sizeof
      in
