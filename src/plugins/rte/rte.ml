...
let mult_sub_add_assertion ~signed ~remove_trivial ~on_alarm (exp,op,lexp,rexp) =
  ...
  if remove_trivial then begin
    ...
    | Some l, Some r, _ -> (* both operands are constant *)
      ...
      in
      (match op with
       | MinusA _ -> warn (Integer.sub l r)
       | PlusA _ -> warn (Integer.add l r)
       | Mult _ -> warn (Integer.mul l r)
       | _ -> assert false)

    | _, Some v , PlusA _ | Some v, _, PlusA _ ->
    ...
    | Some v, None, Mult _ | None, Some v, Mult _
