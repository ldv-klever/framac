...
let rec cc_exp (map:map) exp =
  ...
  | CastE(ty,_,e) -> cast ty (cc_exp map e)
...
let cc_skind map stmt =
  ...
  | Return(None,_) | Goto _ | AsmGoto _ | Break _ | Continue _ | Loop _
...
and cc_term_value (map:map) (term:term) =
  ...
  | Tpif _ | Toffset_min _ | Toffset_max _ | TCoerce _ | TCoerceE _ ->
